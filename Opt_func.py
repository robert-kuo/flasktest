...
def DeleteTask(mainpath, taskname, dirname):
    ret = 200
    try:
        spath = mainpath + '\\' + taskname + ('' if dirname == '' else '\\') + dirname
...
def StageisProcessing(mainpath, taskname, dirname):
    spath = mainpath + '\\' + taskname + '\\' + dirname
    sfile = spath + '\\StageParameter.json'
    ret = False
...
def Get_LastLearningNo(mainpath, taskname):
    lst = os.listdir(mainpath + '\\' + taskname)
    i = 0
    for x in lst:
...
def Evaluate(mainpath, taskname):
    ...
    for x in lst_file:
        if x['Attribute'] == 'Calendar':
            cfile = mainpath + '\\' + taskname + '\\' + x['FileName']
            if not os.path.isfile(cfile): cfile = ''
        elif  x['Attribute'] == 'Setting':
            sfile = mainpath + '\\' + taskname + '\\' + x['FileName']
...
def SaveData_toDataset(mainpath, taskname, demand_start, demand_end, begin_day, end_day, df_orders, df_products, df_lines, df_molds, df_orderdata):
    dirname = mainpath + '\\' + taskname + '' + '\\Dataset'
    if not os.path.isdir(dirname):
        os.makedirs(dirname)
        TaskConfig_SaveFileAttrib(mainpath, taskname, '', 'Dataset', 'Dataset')
    df_orderdata.to_csv(dirname + '\\orderdata.csv', index=False)
    df_products.to_csv(dirname + '\\products.csv', index=False)
    df_lines.to_csv(dirname + '\\lines.csv', index=False)
    df_molds.to_csv(dirname + '\\molds.csv', index=False)
    df_orders.to_csv(dirname + '\\orders.csv', index=False)
    para_data = {'demand_start': str(demand_start), 'demand_end': str(demand_end), 'begin_day': begin_day, 'end_day': end_day}
    with open(dirname + '\\parameter.json', 'w') as fn_json:
        json.dump(para_data, fn_json, ensure_ascii=False)

def LoadData_FromDataset(mainpath, taskname):
    dirname = mainpath + '\\' + taskname + '' + '\\Dataset'
    if os.path.isdir(dirname):
        df_orderdata = pd.read_csv(dirname + '\\orderdata.csv')
        df_products = pd.read_csv(dirname + '\\products.csv')
        df_lines = pd.read_csv(dirname + '\\lines.csv')
        df_molds = pd.read_csv(dirname + '\\molds.csv')
        df_orders = pd.read_csv(dirname + '\\orders.csv')
        ...
        f = open(dirname + '\\parameter.json', 'r')
...
def ExtractData(mainpath, taskname):
    ...
    if cfile != '' and sfile != '':
        ...
        df_productdata, df_linedata, df_molddata, df_orderdata, dm_start, dm_end = Read_OriginData(cfile, sfile)

        # Merge stock data to products
        df_stockdata = readstock(df_orderdata, dm_start)

        #print('read product', df_stockdata.shape[0])
        df_products = readproducts(df_stockdata, df_productdata)
        df_stockdata = updatestock(df_stockdata, df_products)
        ...
        df_orderdata = updateorderdata(df_products, df_orderdata)
        df_orderdata = pd.merge(df_products, df_orderdata, on=['product_code', 'part_no'])

        # Generate df_orders
        df_lines = readlines(df_linedata, df_products)
        df_molds = readmolds(df_lines, df_molddata)
        df_lines = updatelines(df_lines, df_molds, 0)
        begin_day, end_day, df_orders = readorders(df_orderdata, df_products, df_lines, df_molds)
        SaveData_toDataset(mainpath, taskname, dm_start, dm_end, begin_day, end_day, df_orders, df_products, df_lines, df_molds, df_orderdata)
        return 'OK', 200
    else:
...
def Evaluation(mainpath, taskname, maxline_count):
    warnings.filterwarnings("ignore")
    ret, demand_start, demand_end, begin_day, end_day, df_orders, df_products, df_lines, df_molds, df_orderdata = LoadData_FromDataset(mainpath, taskname)
    lst_onstock_prodcode, lst_onstock_qty, df_orders = Orders_On_Stock(df_products, df_orders)
    df_orders = Orders_Overdue(df_lines, df_orders)
    sfile, ret = Evaluation_Report(mainpath, taskname, demand_start, demand_end, end_day, maxline_count, lst_onstock_prodcode, lst_onstock_qty, df_orders, df_lines, df_molds)
...
def FileList(mainpath, taskname, dirname):
    ...
    lst_file = []
    if os.path.isdir(spath + dirname):
        ret = 200
        lst_dir = os.listdir(spath + dirname)
        for x in lst_dir:
            if os.path.isfile(spath + dirname + '\\' + x): lst_file.append(x)
    else:
        ret = 404
